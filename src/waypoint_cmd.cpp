...
extern int _station_specindex_to_paste;
...
CommandCost CmdBuildRailWaypoint(TileIndex start_tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (flags & DC_EXEC) {
		...
		byte map_spec_index;
		if ((flags & DC_PASTE) && (_station_specindex_to_paste != -1)) {
			assert(_station_specindex_to_paste == 0 || (_station_specindex_to_paste < wp->num_specs && wp->speclist[_station_specindex_to_paste].spec == spec));
			map_spec_index = _station_specindex_to_paste;
		} else {
...
{
	if (IsMainMapTile(tile)) {
		_current_pasting->DoCommand(AsMainMapTile(tile), 0, 0, CMD_BUILD_BUOY | CMD_MSG(STR_ERROR_CAN_T_POSITION_BUOY_HERE));
	} else {
