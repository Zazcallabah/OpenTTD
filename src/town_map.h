...
static inline HouseID GetCleanHouseType(const Tindex &t)
{
 ...
 */
template <typename Tindex>
static inline HouseID GetHouseType(const Tindex &t)
{
 ...
 */
template <typename Tindex>
static inline void SetHouseType(const Tindex &t, HouseID house_id)
{
	assert(IsTileType(t, MP_HOUSE));
 ...
 */
template <typename Tindex>
static inline bool IsHouseCompleted(const Tindex &t)
{
	assert(IsTileType(t, MP_HOUSE));
 ...
 */
template <typename Tindex>
static inline void SetHouseCompleted(const Tindex &t, bool status)
{
	assert(IsTileType(t, MP_HOUSE));
 ...
 */
template <typename Tindex>
static inline byte GetHouseBuildingStage(const Tindex &t)
{
	assert(IsTileType(t, MP_HOUSE));
 ...
 */
template <typename Tindex>
static inline byte GetHouseConstructionTick(const Tindex &t)
{
	assert(IsTileType(t, MP_HOUSE));
 ...
 */
template <typename Tindex>
static inline byte GetHouseRandomBits(const Tindex &t)
{
	assert(IsTileType(t, MP_HOUSE));
	return GetTile(t)->m1;
}
 ...
 */
template <typename Tindex>
static inline void MakeHouseTile(const Tindex &t, TownID tid, byte counter, byte stage, HouseID type, byte random_bits)
{
	...
	if (IsMainMapTile(t)) {
		SetAnimationFrame(AsMainMapTile(t), 0);
		SetHouseProcessingTime(AsMainMapTile(t), HouseSpec::Get(type)->processing_time);
	}
}
