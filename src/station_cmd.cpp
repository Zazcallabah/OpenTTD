...
static inline CommandCost GetStationAround(TileArea ta, BaseStation **st, bool waypoint, byte radius = 1)
{
	...
	if (waypoint) {
		Waypoint *waypoint;
		ret = GetStationAround(ta, INVALID_STATION, &waypoint, radius);
		*st = waypoint;
	} else {
		Station *station;
		ret = GetStationAround(ta, INVALID_STATION, &station, radius);
		*st = station;
	}
	return ret;
}
...
CommandCost FindJoiningBaseStation(StationID existing_station, StationID station_to_join, bool adjacent, TileArea ta, T **st)
{
	...
	if (*st == NULL && station_to_join != INVALID_STATION) {
		...
		if (!_settings_game.station.distant_join_stations && (
				...
				GetStationAround(ta, INVALID_STATION, st, 1, station_to_join).Failed() || // search surrounding tiles
	...
	if (flags & DC_EXEC) {
		...
		do {
			...
			do {
				...
				if ((flags & DC_PASTE) && (_station_gfx_to_paste != -1)) {
...
static void CopyPastePlaceRoadStop(GenericTileIndex tile, StationID sid, bool drive_through, RoadStopType rst, RoadTypes rt, DiagDirection dir, bool adjacent)
{
	...
	if (IsMainMapTile(tile)) {
		...
		if (IsRoadStopTile(t) && GetRoadStopType(t) == rst && GetRoadStopDir(t) == dir &&
				 (GetRoadTypes(t) & rt) == rt && CopyPasteCheckOwnership(tile)) {
	...
	if (!with_stations || IsMainMapTile(dst_tile)) {
		switch (station_type) {
			...
			case STATION_BUS: {
				...
				RoadTypes roadtypes = GetRoadTypes(src_tile);
